...
resource "azurerm_subnet" "app_tier_subnet" {
  ...
  address_prefixes     = var.app_tier_subnet_address_prefixes
}
...
resource "azurerm_subnet_network_security_group_association" "nsg_attachment_with_app_tier_subnet" {
...
resource "azurerm_network_security_rule" "app_tier_nsg_rule" {
  for_each                    = local.app_tier_inbound_ports
